...
class APIHandler:
    ...
    async def search_uri(self, uri: str) -> SearchUrisResponse:
        ...
        # create a request object for the web risk google API
        request = SearchUrisRequest()
        request.threat_types = self.THREAT_TYPE  # Specify the type of threat
        request.uri = uri
    ...
    async def process_urls(self):
        urls = self.CVSHandler.load_API_link()  # Load all URLs from CSV

        for url in urls:
            await self.queue.put(url)  # add URLS to the queue

        workers = [asyncio.create.task(self.worker()) for _ in range(APIHandler.CURRENT_LIMIT)]
        await asyncio.gather(*workers)  # wait for workers to finsh
